...
class QuickEditAppender {
  ...
  initCell(row: number, col: number, value: string, vc: VirtualCells) {
    // 從 VC 取得原始狀態的 Cell
    ...
    console.log(typeof now.payload.value);
    return true;
  }
}
...
export function useQuickEditAppender() 
{
  ...
  useEffect(() => {
    ...
    const sub = enterEmitAndRC$.subscribe((payload) => {
      ...
      if(!hasEdited) initSuccess = qea.initCell(row, col, value, cellsVC);
      console.log(value, qea, hasEdited, cellsVC.cellMetaMap.get("Text"));
    });
